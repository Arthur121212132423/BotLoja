...
def salvar_pagamentos():
    try:
        ...
        os.replace(arquivo_temporario, ARQUIVO_PAGAMENTOS)
...
def loja_esta_aberta():
    ...
    return HORA_ABERTURA <= agora.hour < HORA_FECHAMENTO
...
async def atualizar_status_loja():
    ...
    if canal is None:
        try:
            canal = await bot.fetch_channel(CANAL_STATUS_LOJA)
        except Exception as erro:
            print(f"❌ Não consegui encontrar o canal de status: {erro}")
            return

    if not isinstance(canal, discord.TextChannel):
        print(
            f"❌ O ID {CANAL_STATUS_LOJA} não é um canal de texto. "
            f"Tipo encontrado: {type(canal).__name__}"
        )
        return

    novo_nome = "✅│Loja-on" if loja_esta_aberta() else "❌│Loja-off"
    ...
    try:
        ...
        print("❌ Não tenho permissão para alterar o nome do canal de funcionamento.")

    except discord.NotFound:
        print("❌ Canal de funcionamento não encontrado.")

    except Exception as erro:
        print(f"❌ Erro atualizando status da loja: {erro}")
...
def criar_pagamento_pix(
    ...
    external_reference = (
        f"discord-{canal_id}-{usuario_id}-{uuid.uuid4().hex}"
    )
    ...
    dados = {
        "transaction_amount": float(f"{float(preco):.2f}"),
    ...
    print(f"📡 Mercado Pago HTTP: {resposta.status_code}")
    ...
    point_of_interaction = pagamento.get("point_of_interaction", {})
    ...
    print(f"📊 Status detail: {pagamento.get('status_detail')}")
    print(f"📋 QR Code/Pix Copia e Cola: {bool(pix_copia_cola)}")
    print(f"🖼️ QR Base64: {bool(qr_code_base64)}")
    print(f"🔗 Ticket URL: {bool(ticket_url)}")

    if not pix_copia_cola:
        print("❌ Mercado Pago não retornou o Pix Copia e Cola.")
        print(json.dumps(pagamento, indent=4, ensure_ascii=False))
        return None
...
def consultar_pagamento(payment_id: str):
    ...
    if resposta.status_code != 200:
        print(
            f"❌ Erro consultando pagamento {payment_id}: "
...
class EmailPagamentoModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if qr_base64:
            try:
                if qr_base64.startswith("data:image") and "," in qr_base64:
                    qr_base64 = qr_base64.split(",", 1)[1]

                imagem = base64.b64decode(qr_base64, validate=True)
        ...
        try:
            ...
            await interaction.followup.send(
                "❌ Não tenho permissão para enviar o PIX neste ticket.",
                ephemeral=True,
            )
            return
            ...
            await interaction.followup.send(
                "❌ O PIX foi criado, mas não consegui enviar "
                "a mensagem no ticket.",
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            "✅ **PIX gerado!**\n\n"
            "Realize o pagamento pelo **QR Code** ou pelo "
            "**Pix Copia e Cola**.\n\n"
...
class PagamentoView(View):
    ...
    async def pagar_pix(self, interaction, button):
        ...
        produto_id = identificar_produto_do_canal(interaction.channel)
...
async def gerar_transcript(canal: discord.TextChannel):
    linhas = [
        "==================================================",
    ...
    try:
        async for mensagem in canal.history(
            ...
            autor = f"{mensagem.author} (ID: {mensagem.author.id})"
            ...
            for sticker in mensagem.stickers:
                linhas.append(f"[Sticker] {sticker.name}")

            linhas.append("")

    except Exception as erro:
        linhas.append(f"[ERRO AO LER MENSAGENS] {erro}")
...
async def fechar_ticket(
    ...
    for parte in topic.split("|"):
        ...
        if parte.startswith("Cliente:"):
            try:
                cliente_id = int(parte.split(":", 1)[1].strip())
            except ValueError:
                cliente_id = None
            break
    ...
    if not cliente_id:
        await interaction.edit_original_response(
            content=(
                "❌ Não consegui identificar o cliente deste ticket.\n"
    ...
    if not cliente:
        await interaction.edit_original_response(
            content=(
                "⚠️ Não consegui encontrar o cliente deste ticket.\n\n"
    ...
    try:
        await cliente.send(
            content=(
                f"📄 **Transcript do seu ticket — {NOME_LOJA}**\n\n"
                "Seu ticket foi encerrado pela equipe.\n"
                "O transcript completo do atendimento está anexado abaixo."
            ),
            file=discord.File(
                arquivo,
        ...
        await interaction.edit_original_response(
            content=(
                "⚠️ **Não consegui enviar o transcript no PV do cliente.**\n\n"
                "O ticket **não será excluído** para não perder o histórico."
            )
        )
        return
        ...
        await interaction.edit_original_response(
            content=(
                "⚠️ Ocorreu um erro ao enviar o transcript no PV.\n\n"
                "O ticket **não será excluído**."
            )
        )
        return
    ...
    try:
        ...
        print("❌ O bot não tem permissão para excluir o ticket.")
    except Exception as erro:
...
class AnyDeskTutorialView(View):
    ...
    async def informar_id(self, interaction, button):
        await interaction.response.send_modal(AnyDeskModal())
...
def criar_tutorial_embed():
    ...
    embed.set_footer(text=f"{NOME_LOJA} • Atendimento")
    return embed
...
async def criar_ticket(
    ...
    if bot_member is None:
        await interaction.response.send_message(
            "❌ Não consegui identificar as permissões do bot.",
            ephemeral=True,
        )
        return
    ...
    # Permite que o cargo de atendente veja e responda ao ticket
    cargo_atendente = guild.get_role(CARGO_ATENDENTE)

    if cargo_atendente:
        overwrites[cargo_atendente] = discord.PermissionOverwrite(
            view_channel=True,
            send_messages=True,
            read_message_history=True,
            attach_files=True,
        ...
        print(
            f"⚠️ Cargo de atendente {CARGO_ATENDENTE} não encontrado."
        )
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"💰 **Valor:** R${produto['preco']:.2f}\n"
            "🟡 **Status:** Aguardando pagamento\n\n"
            "Clique em **💳 Pagar com PIX** para gerar seu pagamento.\n\n"
            "Após a confirmação, o acesso será liberado automaticamente."
        ),
        color=discord.Color.gold(),
    )
    ...
    try:
        # Menciona o atendente na abertura do ticket
        mencao_atendente = (
        ...
        await canal.send(
            content=(
                f"{membro.mention} {mencao_atendente}\n\n"
                "📢 **Novo ticket aberto!**\n"
...
class OtimizacaoView(View):
    ...
    async def completa(self, interaction, button):
...
class VitaliciaView(View):
    ...
    async def vitalicia(self, interaction, button):
...
class CursoView(View):
    ...
    async def curso(self, interaction, button):
...
async def processar_pagamento_aprovado(
    ...
    if not canal_id or not user_id or not produto_id:
        print(f"⚠️ Dados incompletos no pagamento {payment_id}.")
        return False
    ...
    if canal is None:
        try:
            ...
            print(f"⚠️ Não consegui encontrar o canal {canal_id}: {erro}")
            return False

    if not isinstance(canal, discord.TextChannel):
        print(f"⚠️ Canal {canal_id} não é canal de texto.")
        return False
    ...
    if membro is None:
        try:
            ...
            print(f"⚠️ Não consegui encontrar o cliente {user_id}: {erro}")
            return False
    ...
    for cargo_id in produto["cargos"]:
        ...
        if not cargo:
            print(f"⚠️ Cargo não encontrado: {cargo_id}")
            falhou_cargo = True
            continue
        ...
        try:
            ...
            print(
                f"❌ Não consegui entregar o cargo {cargo.name}. "
            ...
            print(
                f"❌ Erro entregando cargo {cargo.name}: {erro}"
            )
            falhou_cargo = True

    # Só finaliza a entrega quando todos os cargos necessários
    ...
    try:
        ...
        print(f"⚠️ Erro alterando nome do ticket: {erro}")
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Abaixo está o painel para baixar o AnyDesk "
            "e seguir o tutorial."
        ),
        color=discord.Color.green(),
    )
...
class LojaBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        if not atualizar_status_loja_loop.is_running():
            ...
            print("🏪 Status automático iniciado.")

        if not verificar_pagamentos.is_running():
            verificar_pagamentos.start()
            print("💳 Verificação automática de pagamentos iniciada.")
...
@commands.has_permissions(administrator=True)
async def painel_otimizacao(ctx):
    embed = discord.Embed(
        ...
        description=(
            ...
            "Após escolher, será criado um ticket privado para você."
        ),
        color=discord.Color.blurple(),
    )
...
@commands.has_permissions(administrator=True)
async def painel_vitalicia(ctx):
    embed = discord.Embed(
        title="♾️ OTIMIZAÇÃO VITALÍCIA",
        description=(
            "Tenha acesso à **Otimização Completa Vitalícia**.\n\n"
            "♾️ **Valor: R$60,00**\n\n"
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        ...
        print("Configure a variável DISCORD_TOKEN no Laplace.")

    elif not MERCADOPAGO_ACCESS_TOKEN:
        print("❌ BOT NÃO INICIADO!")
        print(
            "Configure a variável MERCADOPAGO_ACCESS_TOKEN no Laplace."
        )
        ...
        try:
            ...
            print(f"❌ Erro fatal ao iniciar o bot: {erro}")
'''

path = Path("/mnt/data/bot.py")
path.write_text(code, encoding="utf-8")
print(f"Arquivo criado: {path}")
print("Alterações principais:")
print("- Canal de funcionamento: 1544187237648826408")
print("- Cargo atendente: 1489876080260026461")
print("- Ticket menciona o atendente ao abrir")
print("- Cargo atendente recebe acesso ao ticket")
print("- PIX continua automático")
